...
pub async fn register_composition<E: Engine>(
    ...
    let request_map: RegisterChain = 
        ...
        .map_err(|e| HandlerError::BadRequest(
            format!("Failed to deserialize request: {}", e))
        )?;

    runtime.register_composition(&request_map.composition.as_str())
    .map_err(|e| HandlerError::Internal(
        format!("Failed to register composition: {}", e))
    )?;
...
pub async fn register_function<E: Engine>(
    ...
    let path_string = if !request_map.local_path.is_empty() {
        if let Err(err) = std::fs::File::open(&request_map.local_path) {
            return Err(HandlerError::BadRequest(
                format!("Failed to open local path for function binary: {}", err)
            ));
        }
        request_map.local_path.clone()
    } else {
        save_function_binary(&request_map.name, &request_map.binary)
        .map_err(|e| HandlerError::Internal(format!("Failed to save function binary: {}", e)))?
    };
    ...
    match runtime.register_function(
        function_name, engine_type, ctx_size,
