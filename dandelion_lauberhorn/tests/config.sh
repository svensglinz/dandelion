#!/bin/bash

# relative to crate root
export DANDELION_TEST_BINARY_PATH="../machine_interface/tests/data"
export DANDELION_TEST_BINARY_ARCH="x86_64"
export LAUBERHORN_IP="10.0.0.5"
export DANDELION_SERVER="localhost"
export DANDELION_PORT=6000
export LAUBERHORN_PORT=12345
export LAUBERHORN_PROG_NUM=1
export LAUBERHORN_PROG_VER=1
export LAUBERHORN_PROC_NUM=1
export DANDELION_ISOL_TYPE="kvm"
